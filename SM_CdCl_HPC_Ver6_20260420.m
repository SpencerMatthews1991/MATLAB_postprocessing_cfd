...
% 
% Version 6: Addtitional support for Reynolds stresses and wake analysis
% for full AoA sweep. Added processing and visualization to support wake
% collected data for metrics such as Reynolds stress and pressure.
...
for i = 1:numCases
    ...
    try
        ...
        skinf = readmatrix(fullfile(currentDir, 'skin_friction-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        clcdf = readmatrix(fullfile(currentDir, 'cl_cd-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        ...
        cdf = readmatrix(fullfile(currentDir, 'drag_coefficient-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        uuf = readmatrix(fullfile(currentDir, 'uu-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        vvf = readmatrix(fullfile(currentDir, 'vv-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        uvf = readmatrix(fullfile(currentDir, 'uv-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        uuwakef = readmatrix(fullfile(currentDir, 'uu_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        vvwakef = readmatrix(fullfile(currentDir, 'vv_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        uvwakef = readmatrix(fullfile(currentDir, 'uv_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        uwakef = readmatrix(fullfile(currentDir, 'velocityx_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        vwakef = readmatrix(fullfile(currentDir, 'velocityy_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        vmagf = readmatrix(fullfile(currentDir, 'velocitymag_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        staticf = readmatrix(fullfile(currentDir, 'staticpressure_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        totalf = readmatrix(fullfile(currentDir, 'totalpressure_wake_points-rfile.out'), ...
            'FileType', 'text', 'NumHeaderLines', 3);
        ...
        vvData = vvf(:, 1:2);
        uvData = uvf(:, 1:2);
        uuWakeData = uuwakef(:, 1:2);
        vvWakeData = vvwakef(:, 1:2);
        uvWakeData = uvwakef(:, 1:2);
        vmagData = vmagf(:, 1:2);
        uData = uwakef(:, 1:2);
        vData = vwakef(:, 1:2);
        staticData = staticf(:, 1:2);
        totalData = staticf(:, 1:2);
        ...
        if size(uuData, 1) > numValues
            uuData = uuData(end-numValues+1:end, :);
        end
        if size(vvData, 1) > numValues
            vvData = vvData(end-numValues+1:end, :);
        end
        if size(uvData, 1) > numValues
            uvData = uvData(end-numValues+1:end, :);
        end
        if size(uuWakeData, 1) > numValues
            uuWakeData = uuWakeData(end-numValues+1:end, :);
        end
        if size(vvWakeData, 1) > numValues
            vvWakeData = vvWakeData(end-numValues+1:end, :);
        end
        if size(uvWakeData, 1) > numValues
            uvWakeData = uvWakeData(end-numValues+1:end, :);
        end
        if size(vmagData, 1) > numValues
            vmagData = vmagData(end-numValues+1:end, :);
        end
        if size(uData, 1) > numValues
            uData = uData(end-numValues+1:end, :);
        end
        if size(vData, 1) > numValues
            vData = vData(end-numValues+1:end, :);
        end
        if size(staticData, 1) > numValues
            staticData = staticData(end-numValues+1:end, :);
        end
        if size(totalData, 1) > numValues
            totalData = totalData(end-numValues+1:end, :);
        end
        ...
        time_avgUUF = mean(uuData(:, 2));
        time_avgVVF = mean(vvData(:, 2));
        time_avgUVF = mean(uvData(:, 2));
        time_avgUUWakeF = mean(uuWakeData(:, 2));
        time_avgVVWakeF = mean(vvWakeData(:, 2));
        time_avgUVWakeF = mean(uvWakeData(:, 2));
        time_avgVmagF = mean(vmagData(:, 2));
        time_avgUF = mean(uData(:, 2));
        time_avgVF = mean(vData(:, 2));
        time_avgStaticF = mean(staticData(:, 2));
        time_avgTotalF = mean(totalData(:, 2));
        ...
        uu_calc(i) = time_avgUUF;
        vv_calc(i) = time_avgVVF;
        uv_calc(i) = time_avgUVF;
        uuwake_calc(i) = time_avgUUWakeF;
        vvwake_calc(i) = time_avgVVWakeF;
        uvwake_calc(i) = time_avgUVWakeF;
        VelocityX_wake(i) = time_avgUF;
        VelocityY_wake(i) = time_avgVF;
        VelocityMag_wake(i) = time_avgVmagF;
        StaticPressure_wake(i) = time_avgStaticF;
        TotalPressure_wake(i) = time_avgTotalF;
        ...
        vvwake_calc(i) = NaN;
        uvwake_calc(i) = NaN;
        VelocityX_wake(i) = NaN;
        VelocityY_wake(i) = NaN;
        VelocityMag_wake(i) = NaN;
        StaticPressure_wake(i) = NaN;
        TotalPressure_wake(i) = NaN;
    end
end
...
saveas(gcf,'20260410_SM_dataprocess_out.png','png') %Rename me!
...
resultsTable = table(aoaValues', Cl_calc, Cd_calc, ClCd_calc, ...
    ...
    'Cl_solver', 'Cd_solver', 'ClCd_solver'});
writetable(resultsTable, '20260410_AoA_sweep_results.csv'); %Rename me!
...
% Cl vs AoA
subplot(2,2,1)
plot(Il_cl(:,1), Il_cl(:,2), 'w-o', 'LineWidth', 2, 'MarkerSize', 8)
hold on
plot(Kh_cl(:,1),Kh_cl(:,2) , 'm-*', 'LineWidth', 2, 'MarkerSize', 8)
hold on
plot(Li_cl(:,1),Li_cl(:,2) , 'b-^', 'LineWidth', 2, 'MarkerSize', 8)
...
% Cd vs AoA
subplot(2,2,3)
plot(Il_cd(:,1), Il_cd(:,2), 'w-o', 'LineWidth', 2, 'MarkerSize', 8)
hold on
plot(Ku_cd(:,1),Ku_cd(:,2) , 'r-s', 'LineWidth', 2, 'MarkerSize', 8)
hold on
...
% % Cl/Cd vs AoA
subplot(2,2,4)
plot(Il_clcd(:,1), Il_clcd(:,2), 'w-o', 'LineWidth', 2, 'MarkerSize', 8)
hold on
plot(Ku_clcd(:,1),Ku_clcd(:,2) , 'r-s', 'LineWidth', 2, 'MarkerSize', 8)
hold on
...
saveas(gcf,'20260410_SM_dataprocess_comparison.png','png') %Rename me!
...
% uu vs AoA
subplot(2,3,1)
plot(aoaValues, uu_calc, 'c-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Component U (uu) [Pa]')
title('Reynolds Stress (uu) vs AoA')
legend('Matthews', 'Location', 'best')
grid on

% vv AoA
subplot(2,3,2)
plot(aoaValues, vv_calc, 'c-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Component V (vv) [Pa]')
title('Reynolds Stress (vv) vs AoA')
legend('Matthews', 'Location', 'northwest')
grid on

% uv vs AoA
subplot(2,3,3)
plot(aoaValues, uv_calc, 'c-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Component U in V (uv) [Pa]')
title('Reynolds Stress (uv) vs AoA')
legend('Matthews', 'Location', 'northwest')
grid on

% uu wake vs AoA
subplot(2,3,4)
plot(aoaValues, uuwake_calc, 'r-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Component U (uu) [Pa]')
title('Reynolds Stress in the Wake (uu) vs AoA')
legend('Matthews', 'Location', 'northwest')
grid on

% vv wake vs AoA
subplot(2,3,5)
plot(aoaValues, vvwake_calc, 'r-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees) [Pa]')
ylabel('Component V (vv)')
title('Reynolds Stress in the Wake (vv) vs AoA')
legend('Matthews', 'Location', 'northwest')
grid on

% uv vs AoA
subplot(2,3,6)
plot(aoaValues, uvwake_calc, 'r-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Component U in V (uv) [Pa]')
title('Reynolds Stress in the Wake (uv) vs AoA')
legend('Matthews', 'Location', 'southwest')
grid on

saveas(gcf,'20260410_SM_reynolds_stress.png','png') %Rename me!

%% Wake Average Pressure Analysis
figure('Position', [200 200 1200 900])

% Static Pressure vs AoA
subplot(1,2,1)
plot(aoaValues, StaticPressure_wake, 'c-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Static Pressure (Pa)')
title('Wake Static Pressure (Pa) vs AoA')
legend('Static Pressure', 'Location', 'northeast')
grid on

% Total Pressure vs AoA 
subplot(1,2,2)
plot(aoaValues, TotalPressure_wake, 'r-o', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Total Pressure (Pa)')
title('Wake Total Pressure (Pa) vs AoA')
legend('Total Pressure', 'Location', 'northeast')
grid on

saveas(gcf,'20260410_SM_wake_pressure.png','png') %Rename me!
...
% X Velocity vs AoA
subplot(1,3,1)
plot(aoaValues, VelocityX_wake, 'c-v', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Wake X Velocity (m/s)')
title('Wake X Velocity vs AoA')
legend('X Velocity', 'Location', 'northeast')
grid on

% Y Velocity vs AoA 
subplot(1,3,2)
plot(aoaValues, VelocityY_wake, 'r-o', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Wake Y Velocity (m/s)')
title('Wake Y Velocity vs AoA')
legend('Y Velocity', 'Location', 'northwest')
grid on

% Velocity Magnitude vs AoA 
subplot(1,3,3)
plot(aoaValues, VelocityMag_wake, 'g-s', 'LineWidth', 2, 'MarkerSize', 8)
hold on
xlabel('Angle of Attack (degrees)')
ylabel('Wake Velocity Magnitude (m/s)')
title('Wake Velocity Magnitude vs AoA')
legend('Velocity Magnitude', 'Location', 'northeast')
grid on

saveas(gcf,'20260410_SM_wake_velocity.png','png') %Rename me!
